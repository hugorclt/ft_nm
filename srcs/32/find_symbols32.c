...
char	find_symbols_type32(Elf32_Sym symtab, Elf32_Shdr *sections)
{
	...
	if (ELF32_ST_BIND(symtab.st_info) == STB_GNU_UNIQUE)
		c = 'u';
	if (ELF32_ST_BIND(symtab.st_info) == STT_GNU_IFUNC)
		c = 'i';
	...
	else if (ELF32_ST_BIND(symtab.st_info) == STB_WEAK && ELF32_ST_TYPE(symtab.st_info) == STT_OBJECT)
	{
		c = 'V';
		if (symtab.st_shndx == SHN_UNDEF)
			c = 'v';
	...
	if (c != '?' && ELF32_ST_BIND(symtab.st_info) == STB_LOCAL) // local symbol are in lowercase
		c += 32;
...
void	find_symbols32(char *data)
{
	...
	Elf32_Shdr *sections = (Elf32_Shdr *)((char *)data + elf->e_shoff);
	Elf32_Shdr *string_table_header = &sections[elf->e_shstrndx];
	...
	for (int i = 0; i < elf->e_shnum; i++) {
		if (sections[i].sh_type == SHT_SYMTAB) {
			Elf32_Sym *symtab = (Elf32_Sym *)((char *)data + sections[i].sh_offset);
			...
			for (int j=0; j<symbol_num; j++) {
				if (symtab[j].st_name) {
					...
					if (ft_strncmp(".debug", string_table + sections[symtab[j].st_shndx].sh_name, 6) == 0 || ft_strncmp(".group", string_table + sections[symtab[j].st_shndx].sh_name, 6) == 0)
						...
						sym_data[len].symbol = find_symbols_type32(symtab[j], sections);
